...
Heap `inuse_space` показывает удерживаемую память, `alloc_space` — суммарные аллокации: большое значение второго ещё не означает утечку. Сбор block/mutex-профилей нужно включать отдельно через `runtime.SetBlockProfileRate` и `runtime.SetMutexProfileFraction`; интенсивное профилирование добавляет накладные расходы. Block profile относится к ожиданиям синхронизации, а не ко всем сетевым задержкам.

Обычные unit/integration-тесты, mocks и coverage — в [конспекте о тестировании Go-сервисов](<31. Тестирование Go-сервисов.md>).
